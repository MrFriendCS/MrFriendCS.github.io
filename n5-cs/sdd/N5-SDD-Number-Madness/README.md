# N5 SDD - Number Madness

## Task

Write a short program that will use two whole numbers.
The program will then show the results of the numbers being:

1. Added
2. Subtracted
3. Multiplied
4. Divided
5. The first raised to the power of the second

Use variables to store the results the above calculations before the answers are displayed.


### Example User Interface

```
Number Madness!
---------------

First value:
10

Second value:
2

Added:
12

Subtracted:
8

Multiplied:
20

Divided:
5.0

To the power of:
100
```


## Testing

1. Use the values shown above.
2. Check the answers your program gives.
3. If the same, use another set of values that you know the answers to, otherwise work out what is wrong.


## Readability

Ensure that your program is easy to understand.  Use:

1. Meaningful identifiers (variable names)
2. Internal commentary (comments)
3. Whitespace (blank lines)


## Reminders

1. Division creates a real value.
